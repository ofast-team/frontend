...
Take a look at the following examples: For each function, try to describe its time complexity using Big-O.
...
We can find the sum $ S = (n-1) + (n - 2) + (n - 3) +  \dots  + 2 + 1$, where $S$ is the number of times the inner loop runs.
...
Therefore, $S = \frac{n(n - 1)}{2}$ or $\frac{n^2 - n}{2}$
