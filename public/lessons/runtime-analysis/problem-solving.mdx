...
long as the Big-O of the solution is small enough. However, collegiate programming contests like the ICPC won't be as forgiving.
We (the O(fast) team) highly recommend learning C++ for competitive programming. C++ can be difficult to troubleshoot sometimes, but its syntax is concise and it's performance is unmatched.
